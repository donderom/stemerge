...
stemerge_de_test() ->
    {Words, Stems} = vocs("de"),
    ?assertEqual(stem_all(fun stemerge_de:stem/1, Words, Stems), []).
...
stemerge_pt_test() ->
    {Words, Stems} = vocs("pt"),
    ?assertEqual(stem_all(fun stemerge_pt:stem/1, Words, Stems), []).

%% italian
stemerge_it_test() ->
    {Words, Stems} = vocs("it"),
    ?assertEqual(stem_all(fun stemerge_it:stem/1, Words, Stems), []).
